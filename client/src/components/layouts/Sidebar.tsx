...
const ItemContent: FC<ItemContentProps> = ({ item, isCollapsed }) => (
  <div className={`flex items-center ${isCollapsed ? "gap-0" : "gap-3"}`}>
    ...
    <span
      className={`${isCollapsed ? "hidden" : "flex-1"} duration-200 transition-colors`}
    >
      {item.label}
    </span>
    {item.badge ? (
      <span
        className={`text-xs bg-global-redpink text-light-bg px-2 py-0.5 rounded-full shadow ${isCollapsed ? "hidden" : ""}`}
      >
        {item.badge}
      </span>
    ) : null}
...
const Sidebar: FC = () => {
            ...
            >
              <div className="relative shrink-0">
                <img
                  ...
                  loading="lazy"
                  decoding="async"
                  className={`${collapsed ? "w-12 h-12" : "w-10 h-10"} object-cover rounded-full`}
                />
              </div>
              {!collapsed && (
  ...
  return (
    <>
      ...
      {/* ======================== */}
      <div
        className={`hidden lg:flex lg:flex-col h-screen sticky top-0 ${baseAsideClass} ${widthClass}`}
      >
        {/* top */}
