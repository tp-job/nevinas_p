...
const FaqNewsFooter: React.FC = () => {
  ...
  return (
    <div className="bg-[#f2f0ec] text-[#141210] flex flex-col w-full overflow-hidden">
      
      {/* META BAR */}
      <div className="grid grid-cols-4 border-b border-black/10">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="px-4 py-2 border-r border-black/10 last:border-r-0 flex items-center gap-2 font-[var(--fm)] text-[0.5rem] tracking-[0.2em] uppercase text-black/30">
            <div className="w-1.5 h-1.5 rounded-full bg-[#e85c2a]" />
            B0{i} ↑
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_1.7fr_1fr] border-b border-black/10 min-h-[105px]">
        ...
        <div className="p-6 border-b md:border-b-0 md:border-r border-black/10 flex items-center justify-center bg-white/50">
          <div className="relative w-12 h-12 rounded-full border-[3px] border-black/10 flex items-center justify-center">
            <div className="w-2 h-2 rounded-full bg-[#e85c2a]" />
            <div className="absolute -left-2 w-3.5 h-4.5 rounded bg-black/10" />
            <div className="absolute -right-2 w-3.5 h-4.5 rounded bg-black/10" />
          </div>
        </div>
        <div className="p-6 md:p-8 border-b md:border-b-0 md:border-r border-black/10 flex flex-col justify-center">
          <div className="font-[var(--fm)] text-sm font-semibold mb-1 flex items-center gap-1.5">
            <span className="text-[#e85c2a]">•</span> Most Common Questions
          </div>
      ...
      <div className="border-b border-black/10">
        {faqData.map((item, idx) => {
          ...
          return (
            <div 
              key={idx} 
              className={`border-t border-black/5 overflow-hidden transition-colors duration-200 cursor-pointer ${isOpen ? 'bg-white/65' : 'hover:bg-white/50'}`}
            ...
            >
              <div className="flex items-center justify-between p-4 md:p-6 gap-4">
                ...
                <button 
                  className={`w-6 h-6 rounded-full border-[1.5px] shrink-0 flex items-center justify-center text-lg transition-all duration-300 ${
                    isOpen ? 'bg-[#e85c2a] border-[#e85c2a] text-white rotate-45' : 'border-black/20 text-black hover:bg-black/5'
                  }`}
                >
                  +
                </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.35fr_1fr_1fr] border-b border-black/10">
        <div className="p-6 md:p-8 border-b lg:border-b-0 lg:border-r border-black/10 flex flex-col justify-between">
          ...
          <button className="w-8 h-8 rounded-full border-[1.5px] border-black/20 flex items-center justify-center text-sm hover:bg-[#e85c2a] hover:border-[#e85c2a] hover:text-white transition-colors mt-4">
            →
          </button>
        </div>
        
        <div className="hidden lg:flex flex-col items-center justify-center gap-2 border-r border-black/10">
          <button className="w-6 h-6 rounded-full border border-black/20 flex items-center justify-center text-[0.65rem] text-black/40 hover:bg-black hover:text-white transition-colors">‹</button>
          <button className="w-6 h-6 rounded-full border border-black/20 flex items-center justify-center text-[0.65rem] text-black/40 hover:bg-black hover:text-white transition-colors">›</button>
        </div>
        ...
        <div className="p-5 md:p-6 border-b lg:border-b-0 lg:border-r border-black/10 flex flex-col gap-2 hover:bg-white/55 transition-colors cursor-pointer group">
          <div className="w-full h-16 rounded-md overflow-hidden relative shrink-0 bg-gradient-to-br from-[#1e202c] to-[#293556]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[0.75fr_1.5fr_0.4fr_0.65fr] border-b border-black/10 min-h-[52px] items-stretch">
        <div className="p-4 md:p-6 border-b md:border-b-0 md:border-r border-black/10 flex items-center">
          <div className="font-[var(--fd)] text-sm font-semibold tracking-tight flex items-center gap-2">
            <div className="w-5 h-5 bg-gradient-to-br from-[#3E60C1] to-[#50409A] rounded flex items-center justify-center shrink-0">
        ...
        <div className="p-4 md:p-6 border-b md:border-b-0 md:border-r border-black/10 flex items-center w-full">
          <div className="flex flex-col gap-1 w-full">
            ...
            <input type="email" placeholder="Your e-mail" className="w-full bg-transparent border-b border-black/20 py-1 font-[var(--fm)] text-sm focus:outline-none focus:border-[#e85c2a] transition-colors placeholder:text-black/20" />
          </div>
        </div>
        ...
        <button className="p-4 md:p-0 bg-[#e85c2a] hover:bg-[#c74a1e] transition-colors flex items-center justify-center text-white font-[var(--fm)] text-[0.65rem] font-bold tracking-[0.12em] uppercase w-full h-full">
          SUBSCRIBE
        </button>
      </div>
          ...
          <div key={i} className="p-4 md:p-6 md:border-r border-black/5 last:border-r-0">
            ...
            <div className="flex flex-col gap-1">
              {col.links.map((link, j) => (
                <a key={j} href="#" className="flex justify-between items-center font-[var(--fm)] text-[0.65rem] text-black/50 hover:text-[#e85c2a] transition-colors py-0.5 group">
                  {link}
      ...
      <div className="p-4 md:p-8 pt-6 relative flex items-end justify-center sm:justify-between overflow-hidden">
        {/* Corners */}
        <div className="absolute top-6 left-6 w-3 h-3 border-t-2 border-l-2 border-[#e85c2a] hidden sm:block" />
        <div className="absolute top-6 right-6 w-3 h-3 border-t-2 border-r-2 border-[#e85c2a] hidden sm:block" />
        <div className="absolute bottom-0 left-6 w-3 h-3 border-b-2 border-l-2 border-[#e85c2a] hidden sm:block" />
        <div className="absolute bottom-0 right-6 w-3 h-3 border-b-2 border-r-2 border-[#e85c2a] hidden sm:block" />
        
        <div 
          className="font-[var(--fd)] font-black tracking-[-0.06em] leading-[0.88] uppercase text-[clamp(4.5rem,14vw,13rem)]"
        >
          ネヴィ<span className="text-[#e85c2a]">ナス</span>
        </div>
      </div>
