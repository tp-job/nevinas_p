...
`;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      render() {
        const velAbs = Math.abs(velocityRef.current);
        ...
        if (sim) {
          sim.options.mouse_force = mouseForce + velocityFactor * 30;
          if (isViscous) {
            sim.options.viscous = viscous * (1.0 - velocityFactor * 0.5);
          }
        }

        // Apply scroll pull force (negative Y direction when scrolling down)
        Mouse.scrollDiffY = velocityRef.current * -0.00015;
