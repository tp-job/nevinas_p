...
// SVG Arc Gauge
const ArcGauge: FC<{ percentage: number; color: string; size?: number }> = ({
  percentage,
  color,
  size = 64,
}) => {
  ...
  return (
    <div className="relative" style={{ width: size, height: size / 2 + 8 }}>
      ...
      >
        ...
        <path
          d={`M ${strokeWidth / 2 + 1} ${center} A ${radius} ${radius} 0 0 1 ${size - strokeWidth / 2 - 1} ${center}`}
          fill="none"
