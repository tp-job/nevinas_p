...
const BlogPostPage: FC<{ post: BlogPost; onBack: () => void }> = ({
  post,
  onBack,
}) => {
  return (
    <div className="min-h-screen bg-light-bg dark:bg-dark-bg animate-in fade-in duration-300">
      ...
      <div className="relative h-[60vh] w-full">
        ...
        <div className="absolute inset-0 flex flex-col justify-end max-w-4xl mx-auto px-6 pb-12">
          <button
            onClick={onBack}
            className="absolute top-8 left-6 md:left-0 flex items-center text-light-surface/80 hover:text-light-surface transition-colors bg-dark-bg/20 hover:bg-dark-bg/40 px-4 py-2 rounded-full backdrop-blur-md"
          >
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <article className="lg:col-span-11">
          <div className="prose prose-lg prose-blue max-w-none text-light-text dark:text-dark-text leading-relaxed">
            <p className="text-xl text-light-text-secondary dark:text-dark-text-secondary font-light leading-relaxed mb-8 border-l-4 border-light-surface-2 dark:border-dark-surface pl-4">
              {post.excerpt}
            </p>
          ...
          <div className="mt-12 flex gap-2">
            {["Technology", "Web", "Development"].map((tag) => (
              <span
                key={tag}
                className="px-3 py-1 bg-light-surface text-dark-surface rounded-full text-sm hover:bg-light-surface-2 cursor-pointer"
              >
                #{tag}
              </span>
            ))}
