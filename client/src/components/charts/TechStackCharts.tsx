...
const DonutChart: FC<{
  ...
  const arcs = data.map((item) => {
    ...
    const x1o = cx + outerR * Math.cos(startAngle),
      y1o = cy + outerR * Math.sin(startAngle);
    const x2o = cx + outerR * Math.cos(endAngle),
      y2o = cy + outerR * Math.sin(endAngle);
    const x1i = cx + innerR * Math.cos(endAngle),
      y1i = cy + innerR * Math.sin(endAngle);
    const x2i = cx + innerR * Math.cos(startAngle),
      y2i = cy + innerR * Math.sin(startAngle);
    ...
    <div className="relative" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <circle
          ...
          r={innerR + 1}
          fill="none"
          className="stroke-light-surface dark:stroke-dark-bg"
          strokeWidth="2"
        />
      </svg>
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-2xl font-extrabold tracking-tight text-light-text dark:text-dark-text">
          {centerValue || total}
        </span>
        <span className="text-[10px] font-medium text-light-text-secondary dark:text-dark-text-secondary mt-0.5">
          {centerLabel || "Total"}
        </span>
      </div>
    </div>
...
const TechStackCharts: FC = () => {
  ...
  topRepos.forEach((r) => {
    (r.topics || []).forEach((t) => {
      ...
      if (["react", "reactjs"].includes(name))
        frameworkMap.set("React", (frameworkMap.get("React") || 0) + 1);
      else if (["tailwindcss", "tailwind"].includes(name))
        frameworkMap.set("Tailwind", (frameworkMap.get("Tailwind") || 0) + 1);
      else if (["nodejs", "node"].includes(name))
        frameworkMap.set("Node.js", (frameworkMap.get("Node.js") || 0) + 1);
      else if (["mongodb", "mongoose"].includes(name))
        frameworkMap.set("MongoDB", (frameworkMap.get("MongoDB") || 0) + 1);
      else if (["express"].includes(name))
        frameworkMap.set("Express", (frameworkMap.get("Express") || 0) + 1);
      else if (["python"].includes(name))
        frameworkMap.set("Python", (frameworkMap.get("Python") || 0) + 1);
      else if (["fullstack"].includes(name))
        frameworkMap.set(
          "Full-Stack",
          (frameworkMap.get("Full-Stack") || 0) + 1,
        );
    });
  });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      ...
      <div className={`p-6 sm:p-8 ${cardCls}`}>
        <div
          ...
          style={{
            background: `linear-gradient(90deg, transparent, ${C.azure}60, ${C.orchid}60, transparent)`,
          }}
        />
      ...
      <div className={`p-6 sm:p-8 ${cardCls}`}>
        <div
          className="absolute top-0 left-0 right-0 h-[2px]"
          style={{
            background: `linear-gradient(90deg, transparent, ${C.orchid}60, ${C.flamingo}60, transparent)`,
          }}
        />
