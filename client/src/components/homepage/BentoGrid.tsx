...
const BentoGrid: React.FC = () => {
  ...
  const c1_opacity = useTransform(progress, [0.26, 0.36, 0.64, 0.74], [0, 1, 1, 0]);
  const c1_y = useTransform(progress, [0.26, 0.36, 0.64, 0.74], [30, 0, 0, -30]);

  const c2_opacity = useTransform(progress, [0.28, 0.38, 0.62, 0.72], [0, 1, 1, 0]);
  const c2_y = useTransform(progress, [0.28, 0.38, 0.62, 0.72], [30, 0, 0, -30]);

  const c3_opacity = useTransform(progress, [0.30, 0.40, 0.60, 0.70], [0, 1, 1, 0]);
  const c3_scale = useTransform(progress, [0.30, 0.40, 0.60, 0.70], [0.94, 1, 1, 0.94]);
  const c3_y = useTransform(progress, [0.30, 0.40, 0.60, 0.70], [30, 0, 0, -30]);

  const c4_opacity = useTransform(progress, [0.32, 0.42, 0.58, 0.68], [0, 1, 1, 0]);
  const c4_y = useTransform(progress, [0.32, 0.42, 0.58, 0.68], [30, 0, 0, -30]);

  const c5_opacity = useTransform(progress, [0.34, 0.44, 0.56, 0.66], [0, 1, 1, 0]);
  const c5_y = useTransform(progress, [0.34, 0.44, 0.56, 0.66], [30, 0, 0, -30]);

  const c6_opacity = useTransform(progress, [0.36, 0.46, 0.54, 0.64], [0, 1, 1, 0]);
  const c6_y = useTransform(progress, [0.36, 0.46, 0.54, 0.64], [30, 0, 0, -30]);

  const c7_opacity = useTransform(progress, [0.38, 0.48, 0.52, 0.62], [0, 1, 1, 0]);
  const c7_y = useTransform(progress, [0.38, 0.48, 0.52, 0.62], [30, 0, 0, -30]);

  const c8_opacity = useTransform(progress, [0.40, 0.50, 0.50, 0.60], [0, 1, 1, 0]);
  const c8_scale = useTransform(progress, [0.40, 0.50, 0.50, 0.60], [0.96, 1, 1, 0.96]);

  const c9_opacity = useTransform(progress, [0.29, 0.39, 0.61, 0.71], [0, 1, 1, 0]);
  const c9_y = useTransform(progress, [0.29, 0.39, 0.61, 0.71], [30, 0, 0, -30]);

  const c10_opacity = useTransform(progress, [0.31, 0.41, 0.59, 0.69], [0, 1, 1, 0]);
  const c10_y = useTransform(progress, [0.31, 0.41, 0.59, 0.69], [30, 0, 0, -30]);

  const c11_opacity = useTransform(progress, [0.33, 0.43, 0.57, 0.67], [0, 1, 1, 0]);
  const c11_y = useTransform(progress, [0.33, 0.43, 0.57, 0.67], [30, 0, 0, -30]);

  const staticCardAnimation = prefersReducedMotion ? undefined : { initial: { opacity: 0, y: 20 }, whileInView: { opacity: 1, y: 0 }, viewport: { once: true } };
  ...
  const AnimatedCard = ({
    ...
    return (
      <motion.div className={className} {...staticCardAnimation} transition={delay ? { delay } : undefined}>
        {children}
      </motion.div>
    );
  };
    ...
    <section className="bg-light-bg dark:bg-dark-bg text-light-text dark:text-dark-text w-full py-10 px-4 sm:py-14 sm:px-6 lg:py-16 lg:px-10 flex justify-center items-center overflow-hidden">
      <div className="max-w-[1400px] w-full mx-auto bento" aria-label="Feature bento grid">
        <AnimatedCard style={{ opacity: c1_opacity, y: c1_y }} className="bc bc--1 pb-9">
            <div className="text-[11px] tracking-[0.2em] mb-4 text-[var(--color-text-secondary)] font-medium uppercase">SCROLL SMOOTHLY</div>
            ...
            <div className="text-[11px] text-[var(--color-text-secondary)] uppercase tracking-widest mb-1">Performance</div>
            <div className="text-xl font-medium tracking-tight text-[var(--color-text-primary)]">Supervision</div>
        </AnimatedCard>

        <AnimatedCard style={{ opacity: c2_opacity, y: c2_y }} className="bc bc--2" delay={0.1}>
            <div className="text-[11px] text-[var(--color-text-secondary)] uppercase tracking-widest mb-1">Identity</div>
        ...
        <AnimatedCard style={{ opacity: c3_opacity, scale: c3_scale, y: c3_y }} className="bc bc--3 relative overflow-hidden flex flex-col justify-end p-6" delay={0.2}>
            ...
            />
            <div 
              className="absolute inset-0 z-0 opacity-20 backdrop-blur-md pointer-events-none select-none bg-cover bg-center"
              style={{ backgroundImage: `url(${heroArt})` }}
            />
            <div className="relative z-10 text-white">
              <div className="text-[11px] text-white/50 uppercase tracking-widest mb-2 border border-white/20 inline-block px-2 py-0.5 rounded-full">Control your Progress Data</div>
              ...
              <div className="flex items-center gap-1 text-sm">
                ...
                <span className="text-[11px] uppercase tracking-widest text-white/60">Scroll → Explore</span>
                <span className="text-white/40">)</span>
              </div>
            </div>
        </AnimatedCard>

        <AnimatedCard style={{ opacity: c4_opacity, y: c4_y }} className="bc bc--4" delay={0.1}>
            <div className="text-[11px] text-[var(--color-text-secondary)] uppercase tracking-widest mb-1">Navigation</div>
        ...
        <AnimatedCard style={{ opacity: c5_opacity, y: c5_y }} className="bc bc--5" delay={0.2}>
            <div className="text-[11px] text-[var(--color-text-secondary)] uppercase tracking-widest mb-1">Typography</div>
        ...
        <AnimatedCard style={{ opacity: c6_opacity, y: c6_y }} className="bc bc--6 dark relative overflow-hidden">
            <div 
            ...
            <div className="relative z-10">
              <div className="text-[11px] text-white/40 uppercase tracking-widest mb-1">Performance</div>
        ...
        <AnimatedCard style={{ opacity: c7_opacity, y: c7_y }} className="bc bc--7 navy" delay={0.1}>
            <div className="text-[11px] text-white/40 uppercase tracking-widest mb-1">Projects</div>
            ...
            <div className="mt-2 p-2 bg-[var(--color-matte-azure)]/10 rounded-md border border-[var(--color-matte-azure)]/20">
              <div className="text-[11px] text-white/40 uppercase tracking-[0.12em]">LMS SYSTEM</div>
        ...
        <AnimatedCard style={{ opacity: c9_opacity, y: c9_y }} className="bc bc--9">
            ...
            <div className="text-[11px] font-medium tracking-widest text-[var(--color-text-primary)] border border-[var(--color-border-secondary)] inline-block px-2 py-0.5 bg-[var(--color-surface-secondary)] shadow-sm rounded-sm">CURSOR CONTENT</div>
            <div className="text-xs text-[var(--color-text-secondary)] mt-2">Cursor Tracking of any kind</div>
        </AnimatedCard>

        <AnimatedCard style={{ opacity: c10_opacity, y: c10_y }} className="bc bc--10" delay={0.1}>
        ...
        <AnimatedCard style={{ opacity: c11_opacity, y: c11_y }} className="bc bc--11 flex flex-col justify-center items-center text-center" delay={0.2}>
