...
const ACCESS_KEY = process.env.NEXT_PUBLIC_WEB3FORMS_KEY ?? '14b47642-77cc-4f20-8c69-4cc2ea27a7f1';
...
const ContactSplit: React.FC = () => {
  ...
  // ─── Render ────────────────────────────────────────────────────────────────
  return (
    <div className="w-full bg-[var(--color-bg-primary)] text-[var(--color-text-primary)] flex flex-col md:flex-row min-h-screen">
      ...
      <div className="w-full md:w-[60%] p-8 sm:p-12 lg:p-20 flex flex-col justify-center relative">
        ...
        >
          <div className="font-[var(--fd)] text-5xl sm:text-6xl lg:text-7xl font-bold tracking-tight leading-none mb-4">
            Nocturnal<br />Contact<span style={{ color: 'var(--color-matte-azure)' }}>.</span>
          </div>
          <div className="font-[var(--fj)] text-lg text-[var(--color-text-secondary)] tracking-widest">
            お問い合わせ
          </div>
        </motion.div>
        ...
        >
          ...
          {/* Name */}
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-6 border-b border-[var(--color-border-subtle)] pb-2 relative group">
            <div className="font-[var(--fm)] text-xs tracking-widest uppercase text-[var(--color-text-muted)] w-24 shrink-0 mt-2 sm:mt-0" style={{ transition: 'color 0.2s' }}>
              Name
            </div>
            <input
              type="text"
              ...
              className="bg-transparent border-none outline-none text-xl sm:text-2xl font-light w-full placeholder:text-[var(--color-text-muted)] disabled:opacity-40 disabled:cursor-not-allowed transition-opacity"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-6 border-b border-[var(--color-border-subtle)] pb-2 relative group">
            ...
            <div className="flex items-center w-full">
              <span className="text-xl sm:text-2xl font-light text-[var(--color-text-muted)] mr-1">@</span>
              <input
                type="email"
                name="email"
                required
          ...
          {/* Project */}
          <div className="flex flex-col sm:flex-row items-start gap-2 sm:gap-6 border-b border-[var(--color-border-subtle)] pb-2 relative group">
            <div className="font-[var(--fm)] text-xs tracking-widest uppercase text-[var(--color-text-muted)] w-24 shrink-0 mt-3" style={{ transition: 'color 0.2s' }}>
              Project
            </div>
            <textarea
              name="message"
              ...
              rows={2}
              className="bg-transparent border-none outline-none text-xl sm:text-2xl font-light w-full placeholder:text-[var(--color-text-muted)] resize-none pt-1 disabled:opacity-40 disabled:cursor-not-allowed transition-opacity"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mt-8">
            ...
            <div
              aria-live="polite"
              aria-atomic="true"
              className="font-[var(--fm)] text-[0.65rem] leading-relaxed text-[var(--color-text-muted)]"
            >
              <AnimatePresence mode="wait">
                {status === 'idle' && (
                  ...
                  >
                    ...
                    <span className="text-[var(--color-text-secondary)]">Name, Email, Project idea, Deadline.</span>
                  </motion.div>
                )}
                  ...
                  >
                    <span className="flex gap-1 items-center" aria-hidden="true">
                      {[0, 1, 2].map((i) => (
                        <motion.span
                          key={i}
                          className="block w-1 h-1 rounded-full bg-[var(--color-matte-azure)]"
                          animate={{ opacity: [0.3, 1, 0.3], scale: [0.8, 1.2, 0.8] }}
                          transition={{ duration: 1, delay: i * 0.18, repeat: Infinity }}
                        />
                      ))}
                    </span>
                    <span style={{ color: 'var(--color-matte-azure)' }}>Transmitting your message...</span>
                  </motion.div>
                )}
                ...
                {status === 'success' && (
                  ...
                  >
                    ...
                    <span style={{ color: '#4ade80' }}>Message received. We'll be in touch.</span>
                  </motion.div>
                )}
            ...
            <motion.button
              ...
              style={{
                backgroundColor:
                  ...
                  : status === 'error'   ? '#dc2626'
                  : 'var(--color-matte-azure)',
              }}
              className="hover:opacity-90 text-white px-8 py-4 rounded-full font-[var(--fm)] text-xs font-bold tracking-widest uppercase transition-all duration-300 whitespace-nowrap disabled:cursor-not-allowed relative overflow-hidden"
      ...
      <div className="w-full md:w-[40%] bg-black/20 p-8 sm:p-12 lg:p-20 relative overflow-hidden flex flex-col">
        ...
        <div className="font-[var(--fm)] text-[0.65rem] tracking-[0.28em] uppercase text-[var(--color-text-muted)] mb-12 relative z-10 shrink-0">
        ...
        >
          <div className="font-[var(--fm)] text-[0.65rem] tracking-[0.22em] uppercase text-[var(--color-text-muted)] mb-4 pt-4 border-t border-[var(--color-border-subtle)]">
            Alternatives
          </div>
          ...
          {[
            ...
            <div
              key={i}
              className="flex items-center justify-between py-4 border-b border-[var(--color-border-subtle)] cursor-pointer group transition-all duration-200 hover:pl-2"
            >
              <div className="font-[var(--fd)] text-xl sm:text-2xl font-light tracking-tight text-[var(--color-text-secondary)] group-hover:text-[var(--color-text-primary)] transition-colors">
                {alt.name}
              </div>
              <div className="font-[var(--fm)] text-[0.65rem] tracking-[0.14em] uppercase text-[var(--color-text-muted)] group-hover:text-[var(--color-matte-azure)] transition-colors flex items-center gap-2">
                {alt.action}
                {alt.icon === 'svg-copy' && (
        ...
        {/* Ambient Kanji */}
        <div
          className="absolute -bottom-8 -right-4 font-[var(--fj)] text-[clamp(8rem,18vw,16rem)] font-light leading-none text-white/[0.025] tracking-[-0.05em] select-none pointer-events-none mix-blend-overlay"
          aria-hidden="true"
        >
          連絡
        </div>
