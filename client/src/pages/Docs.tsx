...
/* ==================== Breadcrumb ==================== */
const Breadcrumb: FC<{ items: { label: string; href?: string }[] }> = ({ items }) => (
    <nav className="flex items-center gap-2 text-sm mb-8 text-light-text-secondary dark:text-dark-text-secondary">
        {items.map((item, i) => (
            <span key={i} className="flex items-center gap-2">
                {i > 0 && <span className="opacity-60">/</span>}
                {item.href ? (
                    <Link to={item.href} className="text-matte-azure hover:underline">{item.label}</Link>
                ) : (
                    <span className="text-light-text dark:text-dark-text">{item.label}</span>
                )}
            </span>
        ))}
    </nav>
...
const DocSection: FC<{
    ...
    <section className="mb-16">
        <h2 className="text-2xl sm:text-3xl font-bold text-light-text dark:text-dark-text mb-2">{title}</h2>
        {subtitle && (
            <p className="text-base text-light-text-secondary dark:text-dark-text-secondary mb-6">{subtitle}</p>
        )}
...
const Callout: FC<{
    ...
    <div className="flex gap-4 p-5 rounded-xl bg-light-surface-2/80 dark:bg-dark-surface/80 border-l-4 border-matte-azure">
        ...
        <div>
            <p className="font-bold text-light-text dark:text-dark-text mb-2">{title}</p>
            <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary leading-relaxed">{children}</p>
        </div>
    </div>
);
...
const Docs: FC = () => {
    ...
    const [selectedRepoData, setSelectedRepoData] = useState<GitHubRepo | null>(null);
    ...
    const projectArchitecture = selectedRepo ? projectDetailByRepo[selectedRepo] : null;
    ...
    return (
        <div className="w-full">
            ...
            <div className="mb-6">
                ...
                <h2 className="mb-1 text-4xl sm:text-5xl text-light-text dark:text-dark-text">Document</h2>
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {/* Header / Introduction */}
                <div className="mb-12">
                    <h1 className="text-4xl sm:text-5xl font-bold text-light-text dark:text-dark-text mb-3">Documentation</h1>
                    ...
                    <p className="text-base text-light-text dark:text-dark-text leading-relaxed mb-8">
                        This portfolio combines React 19, Express 5, and MongoDB into a full-stack application. 
                        The docs cover architecture, API endpoints, data models, and setup instructions for local development.
                    </p>
                    <Callout title="Design principle">
                        Prefer &apos;few, high-quality primitives&apos; over scattered features. If a workflow cannot be explained in one screen, we simplify it.
                    </Callout>
                </div>

                {/* ========== What you can do ========== */}
                <DocSection title="What you can do" subtitle="Explore project documentation and technical details.">
                    <ul className="list-disc pl-6 space-y-3 text-light-text-secondary dark:text-dark-text-secondary">
                        ...
                        <li>Review architecture overview — frontend, backend, and dev tools.</li>
                ...
                <DocSection title="Project Docs" subtitle="คลิกที่ repo card เพื่อดูเอกสาร (README) ของโปรเจกต์นั้น">
                ...
                {reposLoading ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {[...Array(6)].map((_, i) => (
                            <div key={i} className="rounded-2xl p-6 animate-pulse bg-light-surface-2 dark:bg-dark-surface h-40" />
                        ))}
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {!reposLoading && repos.length === 0 && (
                    <p className="text-center py-8 text-light-text-secondary dark:text-dark-text-secondary">No repositories found</p>
                )}
                </div>
                </DocSection>
            ...
            {selectedRepo && (
                <>
                        ...
                        <div className={`p-6 sm:p-8 mb-8 ${cardCls}`}>
                            ...
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    ...
                                    <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary mt-0.5">Technology stack breakdown</p>
                                </div>
                                <button
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                                {[
                                    { title: 'Frontend', icon: 'ri-window-line', color: TH.azure, data: projectArchitecture.frontend },
                                    { title: 'Backend', icon: 'ri-server-line', color: TH.orchid, data: projectArchitecture.backend },
                                    { title: 'Dev Tools', icon: 'ri-tools-line', color: TH.yellow, data: projectArchitecture.devTools },
                                ].map((section) => (
                            ...
                            <div className="flex flex-wrap gap-3">
                                {selectedRepoData.language && (
                                    <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-light-surface-2 dark:bg-dark-surface border border-light-border dark:border-dark-border">
                                        <span className="w-3 h-3 rounded-full" style={{ backgroundColor: LANG_COLORS[selectedRepoData.language] || '#6e7681' }} />
                                        <span className="text-sm font-semibold text-light-text dark:text-dark-text">{selectedRepoData.language}</span>
                                    </div>
                                )}
                                {(selectedRepoData.topics || []).map((t) => (
                                    <span key={t} className="px-3 py-2 rounded-xl bg-light-surface-2 dark:bg-dark-surface border border-light-border dark:border-dark-border text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary">
                                        {t}
                                    </span>
                                ))}
                            </div>
                    ...
                    {readmeError && (
                        <div className="py-12 text-center">
                            ...
                            <p className="text-light-text-secondary dark:text-dark-text-secondary">{readmeError}</p>
                        </div>
                    )}
                    {readmeContent && !readmeLoading && (
                        <div className="prose-doc [&_h1]:text-2xl [&_h1]:font-bold [&_h1]:mb-4 [&_h1]:mt-8 [&_h1]:text-light-text [&_h1]:dark:text-dark-text
                ...
                {/* ========== Architecture ========== */}
                <DocSection title="Architecture Overview" subtitle="Technology stack breakdown">
                <div className={`p-6 sm:p-8 mb-8 ${cardCls}`}>
                    <div className="absolute top-0 left-0 right-0 h-[2px]"
                        style={{ background: `linear-gradient(90deg, transparent, ${TH.azure}60, ${TH.orchid}60, transparent)` }} />
                ...
                {dataModels.filter(m => m.name === openModel).map((model) => (
                    ...
                    {/* ========== Getting Started ========== */}
                <DocSection title="Getting Started" subtitle="Setup guide for local development">
                <div className={`p-6 sm:p-8 mb-8 ${cardCls}`}>
                    <div className="absolute top-0 left-0 right-0 h-[2px]"
                ...
                <div className="space-y-5">
                    ...
                    {/* ========== Folder Structure ========== */}
                <DocSection title="Folder Structure" subtitle="Project directory layout">
                <div className={`p-6 sm:p-8 mb-8 ${cardCls}`}>
                    <div className="absolute top-0 left-0 right-0 h-[2px]"
